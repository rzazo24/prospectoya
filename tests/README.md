...
| `test-busqueda.js` | El buscador completo en jsdom con `fetch` simulado: autocompletado con *debounce*, teclado (↑ ↓ Enter Esc), búsqueda por CN y por nº de registro, aviso con códigos incompletos, CN en diferido (una sola petición por `nregistro`, caché reutilizada), CN de todos los envases en la ficha y la **ventana del detalle** (`<dialog>`): se abre al elegir una sugerencia **y** al pulsar un resultado, la cierra la ✕, la cierra un clic en el fondo y se puede volver a abrir (jsdom no implementa `showModal`, así que se simula; el centrado y el velo van en `test-resoluciones.js`); también los badges de suministro/seguimiento y el enlace al documento oficial (salen y se ocultan según traiga o no el medicamento `psum`/`triangulo`/`docs[]`, y el enlace sigue a la pestaña activa); los **filtros combinables** (receta, comercialización, laboratorio): abrir/cerrar el panel, que elegir un filtro repite la búsqueda con el término ya escrito, que no afectan al desplegable de sugerencias, "Limpiar filtros" y el resaltado del botón; los medicamentos con **documentación reducida** (importaciones paralelas): que `listarSecciones()` normalice a `[]` el `{error:...}` que devuelve la API, que el acordeón no rompa y que el enlace al documento oficial use el PDF cuando no hay `urlHtml`; y **mi botiquín** (guardar/quitar en `localStorage`): la estrella refleja si el medicamento abierto está guardado, el botón "Tu botiquín" despliega/pliega la lista (como "Filtros", sin ocupar sitio si no hace falta), los chips abren (pidiendo datos frescos a la API, nunca lo guardado) y quitan sin pasar por la ficha, y un valor corrupto en `localStorage` no rompe la lectura. | Node + jsdom |
...
| `test-api-real.js` | `api.js` contra la API real de CIMA: `?cn=` exacto, CN inexistente, nº de registro largo (tipo EMA), varios envases por `nregistro`, que `/medicamentos` sigue sin devolver `cn`, paginación, mayúsculas/acentos, `dcp.id`/`vmp`, `nosustituible`, los filtros `receta`/`comerc`/`laboratorio` (incluida su combinación en AND) y que `/docSegmentado/secciones` devuelve `{error:...}` (no `[]`) para un medicamento con documentación reducida. | Red |
